...
void detectAndDisplay( Mat frame, vector<Mat> &images )
{
  ...
  for( size_t i = 0; i < min(faces.size(), (size_t)10); i++ )
  {
	...
	if ((max_idx[0] >= max_idx[2]) && (max_idx[1] >= max_idx[2]) && ((images[i].cols*images[i].rows) > 2500)) {
	   stringstream imName;
	   imName << "/home/alon/FRC900_Vision/detection for empty/detected/nobin_";
	   imName << frameCount;
	   imName << i;
	   imName << ".jpg";
	   string imNameString = imName.str();
	   vector<int> compression;
	   compression.push_back(CV_IMWRITE_JPEG_QUALITY);
	   compression.push_back(10);
	   imwrite(imNameString,images[i],compression);
}
